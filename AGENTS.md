...
- Componentes:
  ...
  - `ProductCatalogApp.jsx` — estado central: sucursal seleccionada (default Trelew), **buscador grande por nombre** (ubicado debajo de los filtros, con **debounce de 300ms** y **guard de secuencia** `searchSeqRef` para no sobreescribir resultados con respuestas de fetches anteriores), **autocompletado de Almacén** (dropdown con sugerencias del rubro Almacén, debounce 250ms, cierre con click fuera/Escape, al seleccionar rellena el buscador con el nombre completo del producto — ver fix #19), filtro "Solo Ofertas" (el estado vacío explica si no hay ofertas que coincidan), filtro por categoría (dropdown con `fetchCategorias`), **lazy load** (renderiza 60 y botón "Cargar más", contador "Mostrando X de Y"). **Sin filtro de precio** (se eliminó "Precio Máximo $" en el fix #17).
  - `ProductCard.jsx` — tarjeta con badge de sucursal, imagen, precios, estado de stock y categoría (**sin badge de descuento %**: el porcentaje solo se ve al abrir "Ver detalles", en el `ProductDetailModal`). Sin badge de SKU ni botón "Historial".
